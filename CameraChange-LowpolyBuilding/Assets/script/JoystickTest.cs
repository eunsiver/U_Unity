...
public class JoystickTest : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
  ...
  void Start()
  {
    ...
    //this.anim = this.go_Player.GetComponent<Animation>();
    //palyerAni = go_Player.GetComponent<Animator>();
  ...
  void Update()
  {
    if (this.isTouch)
    {
      ...
      if (this.value != null)
      {
        this.go_Player.transform.rotation = Quaternion.Euler(0f, Mathf.Atan2(this.value.x, this.value.y) * rotatespd * Mathf.Rad2Deg, 0f);
      }
    }
  }
